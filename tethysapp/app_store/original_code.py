...
from tethys_apps.cli.services_commands import (
...
def create_services(service, create_service, config):
    ...
    try:
        ...
        write_msg(
            'Service with name "{0}" already exists. Skipping add.'.format(
                config["name"]
            )
        )
...
def get_service_from_id(id):
    ...
    try:
        ...
        return {"service_type": "spatial", "linkParam": serviceLinkParam["spatial"]}
    except ObjectDoesNotExist:
        pass

    try:
        entries = DatasetService.objects.get(id=id)  # noqa: F841
        return {"service_type": "dataset", "linkParam": serviceLinkParam["dataset"]}
    except ObjectDoesNotExist:
        pass

    try:
        entries = WebProcessingService.objects.get(id=id)  # noqa: F841
        return {"service_type": "wps", "linkParam": serviceLinkParam["persistent"]}
    except ObjectDoesNotExist:
        pass
...
def get_service_from_name(name):
    ...
    try:
        ...
        return {"service_type": "wps", "linkParam": serviceLinkParam["wps"]}
    except ObjectDoesNotExist:
        pass
...
def run_interactive_services(app_name):
    ...
    write_msg(
        "Please enter the service ID/Name to link one of the above listed service."
    )
...
def find_and_link(service_type, setting_name, service_id, app_name):
    ...
    if service:
        ...
        with pretty_output(FG_RED) as p:
            p.write(
                "Warning: Could not find service of type: {} with the name/id: {}".format(
...
def run_portal_init(service_models, file_path, app_name):
    ...
    if (
        ...
        if services and len(services) > 0:
            ...
            write_msg(
                "No app configuration found for app: {} in portal config file. ".format(
                    app_name
                )
            )
...
def install_dependencies(conda_config, pip_config):
    ...
    if (
        "channels" in conda_config
        and conda_config["channels"]
        and len(conda_config["channels"]) > 0
    ):
    ...
    if (
        "dependencies" in conda_config
        and conda_config["dependencies"]
        and len(conda_config["dependencies"]) > 0
    ):
    ...
    for service_type in service_options:
        if service_options[service_type]:
            for service in service_options[service_type]:
                create_services(
                    service_models[service_type], creators[service_type], service
                )
...
def init_command(args):
    ...
    if "conda" not in init_options:
        with pretty_output(FG_BLUE) as p:
            p.write("No Conda options found. Does your app not have any dependencies?")
        exit(0)
    ...
    if (
        "post" in init_options
        and init_options["post"]
        and len(init_options["post"]) > 0
    ):
        write_msg("Running post installation tasks...")
        source_dir = os.path.dirname(tethysapp_dir)
        for post in init_options["post"]:
            path_to_post = os.path.join(
                os.path.dirname(os.path.realpath(file_path)), post
            )

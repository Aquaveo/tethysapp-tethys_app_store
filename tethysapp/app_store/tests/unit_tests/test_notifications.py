...
from tethysapp.app_store.notifications import (
...
async def test_check_user_permissions(mocker):
    ...
    mocker.patch(
        "tethysapp.app_store.notifications.User.objects.get", return_value=mock_user
    )
...
async def test_check_user_permissions_dne(mocker):
    mocker.patch(
        "tethysapp.app_store.notifications.User.objects.get",
...
async def test_notificationsConsumer_not_authorized(mocker, caplog):
    ...
    with override_settings(CHANNEL_LAYERS=channel_layers_setting):
        communicator = WebsocketCommunicator(
            consumer.as_asgi(), "GET", "install/notifications"
        )
        ...
        assert (
            f"Removed {channel_name} channel from notifications" not in caplog.messages
        )
...
async def test_notificationsConsumer_install_notifications(mocker, caplog):
    ...
    with override_settings(CHANNEL_LAYERS=channel_layers_setting):
        ...
        await channel_layer.group_send(
            "notifications", {"type": "install_notifications", "message": message}
        )
...
async def test_notificationsConsumer_receive_begin_install(mocker, caplog):
    ...
    mocker.patch(
        "tethysapp.app_store.notifications.sync_to_async",
        side_effect=[mock_get_workspace],
    )
    ...
    with override_settings(CHANNEL_LAYERS=channel_layers_setting):
        ...
        assert f"Added {channel_name} channel to notifications" in caplog.messages
        assert (
            f"Received message {json.dumps(install_data)} at {channel_name}"
            in caplog.messages
        )
        assert f"Removed {channel_name} channel from notifications" in caplog.messages
        mock_threading.Thread.assert_called_with(
            target=mock_begin_install,

...
from tethysapp.app_store.installation_handlers import (
    get_service_options,
...
def test_get_service_options(mocker):
    ...
    mock_services_list_command = mocker.patch(
        "tethysapp.app_store.installation_handlers.services_list_command",
...
def test_continueAfterInstall(mocker):
    mock_ws = mocker.patch(
        "tethysapp.app_store.installation_handlers.send_notification"
    )
    mock_dad = mocker.patch(
        "tethysapp.app_store.installation_handlers.detect_app_dependencies"
    )
    app_install_data = {"isInstalled": True, "channel": "channel", "version": "1.0"}
    mocker.patch(
        "tethysapp.app_store.installation_handlers.check_if_app_installed",
        return_value=app_install_data,
    )
...
def test_set_custom_settings(tethysapp, tmp_path, mocker):
    ...
    mocker.patch(
        "tethysapp.app_store.installation_handlers.get_app_instance_from_path",
        return_value=app,
    )
    mock_process_settings = mocker.patch(
        "tethysapp.app_store.installation_handlers.process_settings"
    )
    mocker.patch(
        "tethysapp.app_store.installation_handlers.TethysApp.objects.get",
        return_value=tethysapp_object,
    )
    mock_actual_setting = MagicMock(value="test")
    mocker.patch(
        "tethysapp.app_store.installation_handlers.CustomSetting.objects.get",
        return_value=mock_actual_setting,
    )
    custom_settings_data = {
        "app_py_path": tmp_path,
        "settings": {"mock_setting": "setting_value"},
    }
    mock_channel = MagicMock()
    ...
    mock_ws.assert_has_calls(
        [
            ...
            call(
                {"data": {}, "jsHelperFunction": "customSettingConfigComplete"},
                mock_channel,
            ),
        ]
    )
...
def test_process_settings(tmp_path, tethysapp, mocker):
    ...
    mocker.patch(
        "tethysapp.app_store.installation_handlers.get_app_settings",
        return_value=mock_settings,
    )
    mocker.patch(
        "tethysapp.app_store.installation_handlers.get_service_type_from_setting",
        return_value="spatial",
    )
    mocker.patch(
        "tethysapp.app_store.installation_handlers.get_setting_type_from_setting",
        return_value="ds_spatial",
    )
    service_options = {"name": "spatial_service", "id": 1}
    mocker.patch(
        "tethysapp.app_store.installation_handlers.get_service_options",
        return_value=service_options,
    )
    app = tethysapp()
    mock_channel = MagicMock()
...
def test_process_settings_no_settings(tmp_path, tethysapp, mocker):
    ...
    mocker.patch(
        "tethysapp.app_store.installation_handlers.get_app_settings", return_value=[]
    )
    app = tethysapp()
    mock_channel = MagicMock()
...
def test_configure_services(mocker):
    ...
    mock_link = mocker.patch(
        "tethysapp.app_store.installation_handlers.link_service_to_app_setting"
    )
    mock_channel = MagicMock()
    services_data = {

...
from tethysapp.app_store.proxy_app_handlers import (
    create_proxy_app,
    list_proxy_apps,
    delete_proxy_app,
    update_proxy_app,
...
def test_create_proxy_app_already_exists(mocker, proxy_app_install_data, caplog):
    ...
    mock_sn.assert_called_with(
        f"There is already a proxy app with that name: {app_name}", mock_channel
    )
    assert f"Checking to see if the {app_name} proxy app exists" in caplog.messages
...
def test_delete_proxy_app(mocker):
    ...
    mock_sn.assert_called_with(
        "Proxy app 'test_app' was deleted successfully", mock_channel
    )
...
def test_update_proxy_app(mocker, proxy_app_install_data, proxyapp):
    ...
    mock_sn.assert_called_with(
        "Proxy app 'test proxy app' was updated successfully", mock_channel
    )


def test_update_proxy_app_missing_attr(
    mocker, proxy_app_install_data, proxyapp, caplog
):
    proxyApp = proxyapp()
    mocker.patch("tethys_apps.models.ProxyApp.objects.get", return_value=proxyApp)
    mock_sn = mocker.patch("tethysapp.app_store.proxy_app_handlers.send_notification")
...
def test_submit_proxy_app(mocker, store, proxyapp):
    ...
    mock_submit = mocker.patch(
        "tethysapp.app_store.proxy_app_handlers.submit_proxyapp_to_store"
    )
    app = proxyapp()

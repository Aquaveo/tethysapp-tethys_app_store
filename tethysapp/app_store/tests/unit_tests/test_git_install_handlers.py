...
from tethysapp.app_store.git_install_handlers import (
    update_status_file,
...
def test_run_pending_installs(app_store_workspace, mocker, caplog):
    ...
    mock_continue = mocker.patch(
        "tethysapp.app_store.git_install_handlers.continue_install"
    )
    mocker.patch(
        "tethysapp.app_store.git_install_handlers.get_app_workspace",
        return_value=mock_workspace,
    ...
    install_options = {
        ...
        "requirements": {
            "skip": False,
            "conda": {"channels": "conda-forge", "packages": ["numpy"]},
            "pip": ["requests"],
        },
    }
    workspace_app = str(app_store_workspace / "apps" / "github_installed" / "test_app")
    mock_continue.assert_called_with(
        workspace_app,
        mock_logger,
        str(git_status_file),
        install_options,
...
def test_install_packages(mocker, app_store_workspace):
    mock_conda = mocker.patch(
        "tethysapp.app_store.git_install_handlers.conda_run",
...
def test_continue_install(app_store_workspace, mocker):
    ...
    mock_restart_server = mocker.patch(
        "tethysapp.app_store.git_install_handlers.restart_server"
    )
    mock_clear_github_cache_list = mocker.patch(
        "tethysapp.app_store.git_install_handlers.clear_github_cache_list"
    )
    ...
    assert (
        call(["tethys", "db", "sync"], stdout=mock_pipe, stderr=mock_stdout)
    ...
    mock_restart_server.assert_called_with(
        {"restart_type": "github_install", "name": app_name},
        channel_layer=None,
        app_workspace=str(app_store_workspace),
    )
...
def test_install_worker(app_store_workspace, mocker):
    ...
    mock_install_packages = mocker.patch(
        "tethysapp.app_store.git_install_handlers.install_packages"
    )
    mock_continue = mocker.patch(
        "tethysapp.app_store.git_install_handlers.continue_install"
    )
    ...
    mock_install_packages.assert_called_with(
        conda_config, mock_logger, str(status_file_path)
    )
    git_status = json.loads(status_file_path.read_text())
    assert (
        call(["pip", "install", "requests"], stdout=mock_pipe, stderr=mock_stdout)
        in mock_popen.mock_calls
    )
    assert (
        call(
    ...
    assert (
        call("Python Application install exited with: 0") in mock_logger.info.mock_calls
    )
...
def test_get_log_file(app_store_workspace):
    ...
    assert log_file == str(
        app_store_workspace / "logs" / "github_install" / "abc123.log"
    )
...
def test_get_status_file(app_store_workspace):
    ...
    assert status_file == str(
        app_store_workspace / "install_status" / "github" / "abc123.json"
    )

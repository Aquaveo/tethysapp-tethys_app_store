...
def test_scaffold_command(mock_admin_api_request, mocker, tmp_path):
    ...
    mocker.patch('tethysapp.app_store.scaffold_handler.has_permission', side_effect=[True])
    url = reverse('app_store:scaffold_app')
    data = {
        "name": "new-Name",
    ...
    assert (tmp_path / "develop" / f'tethysapp-{app_name}' / "tethysapp" / app_name).is_dir()
    app_py = tmp_path / "develop" / f'tethysapp-{app_name}' / "tethysapp" / app_name / 'app.py'
...
def test_scaffold_command_bad_project_name(mock_admin_api_request, mocker, tmp_path):
    ...
    assert json_response['message'] == f'Error: Invalid characters in project name "{project_name}". Only letters, numbers, and underscores.'  # noqa: E501
...
def test_scaffold_command_bad_proper_name(mock_admin_api_request, mocker, tmp_path):
    ...
    json_response = json.loads(api_response.content)
    assert json_response['status'] == 'false'
    assert json_response['message'] == 'Error: Proper name can only contain letters and numbers and spaces.'
...
def test_scaffold_command_unable_to_overwrite(mock_admin_api_request, mocker, tmp_path):
    ...
    mocker.patch('tethys_apps.base.workspace.get_app_workspace', return_value=mock_workspace)
    mocker.patch('tethysapp.app_store.scaffold_handler.install_app')
    mocker.patch('tethysapp.app_store.scaffold_handler.shutil.rmtree', side_effect=[OSError])
    mocker.patch('tethysapp.app_store.scaffold_handler.has_permission', side_effect=[True])
    url = reverse('app_store:scaffold_app')
    data = {
        "name": "new-Name",
    ...
    json_response = json.loads(api_response.content)
    assert json_response['status'] == 'false'
    assert json_response['message'] == f'Error: Unable to overwrite {str(project_root)}. Please remove the directory and try again.'  # noqa: E501
    assert (tmp_path / "install_status" / "scaffoldRunning").is_file()
...
def test_scaffold_command_overwrite_false(mock_admin_api_request, mocker, tmp_path):
    ...
    json_response = json.loads(api_response.content)
    assert json_response['status'] == 'false'
    assert json_response['message'] == f'Error: App directory exists {project_root} and Overwrite was not permitted. ' \

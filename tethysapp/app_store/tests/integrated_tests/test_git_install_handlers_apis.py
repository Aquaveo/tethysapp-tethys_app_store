...
def test_get_status_no_permission(mock_admin_api_get_request, mocker, tmp_path):
    ...
    mocker.patch(
        "tethysapp.app_store.git_install_handlers.has_permission", side_effect=[False]
    )
    url = reverse("app_store:git_get_status")
    data = {}
...
def test_get_status_called(mock_admin_api_get_request, mocker, tmp_path):
    ...
    mocker.patch(
        "tethysapp.app_store.git_install_handlers.has_permission", side_effect=[True]
    )
    mock_get_status_main = mocker.patch(
        "tethysapp.app_store.git_install_handlers.get_status_main",
...
@pytest.mark.django_db
def test_get_status_override_no_custom_key_set(
    mock_admin_api_get_request, mocker, tmp_path
):
    mocker.patch(
        "tethysapp.app_store.git_install_handlers.get_app_workspace",
        return_value=str(tmp_path),
    )
    mocker.patch(
        "tethysapp.app_store.git_install_handlers.get_override_key", return_value=None
    )
...
@pytest.mark.django_db
def test_get_status_override_no_custom_key_provided(
    mock_admin_api_get_request, mocker, tmp_path
):
    mocker.patch(
        "tethysapp.app_store.git_install_handlers.get_app_workspace",
    ...
    mocker.patch(
        "tethysapp.app_store.git_install_handlers.get_override_key",
        side_effect=["override_key"],
    )
    url = reverse("app_store:git_get_status_override")
    data = {}
...
def test_get_logs_called(mock_admin_api_get_request, mocker, tmp_path):
    ...
    mock_get_logs_main = mocker.patch(
        "tethysapp.app_store.git_install_handlers.get_logs_main",
        side_effect=[JsonResponse({})],
    )
    url = reverse("app_store:git_get_logs")
    data = {}
...
@pytest.mark.django_db
def test_get_logs_override_no_custom_key_set(
    mock_admin_api_get_request, mocker, tmp_path
):
    mocker.patch(
        "tethysapp.app_store.git_install_handlers.get_app_workspace",
...
@pytest.mark.django_db
def test_get_logs_override_no_custom_key_provided(
    mock_admin_api_get_request, mocker, tmp_path
):
    mocker.patch(
        "tethysapp.app_store.git_install_handlers.get_app_workspace",
...
@pytest.mark.django_db
def test_install_git_already_exists(
    mock_admin_api_post_request, app_store_workspace, mocker, caplog
):
    ...
    mock_install_worker = mocker.patch(
        "tethysapp.app_store.git_install_handlers.install_worker"
    )
    mock_git_install_logger = mocker.patch(
        "tethysapp.app_store.git_install_handlers.git_install_logger"
    )
    url = reverse("app_store:install_git")
    data = {
        "url": "https://github.com/test_app.git",
        "branch": "master",
        "develop": True,
    }
    ...
    assert (
        call(f"Starting GitHub Install. Installation ID: {install_id}")
        in mock_git_install_logger.info.mock_calls
    )
    ...
    assert (
        call(f"Application Install Path: {app_location}")
        in mock_git_install_logger.info.mock_calls
    )
    assert (
        call("Git Repo exists locally. Doing a pull to get the latest")
        in mock_git_install_logger.info.mock_calls
    )
    ...
    mock_threading.Thread.assert_called_with(
        target=mock_install_worker,
        name="InstallApps",
        args=(app_location, status_file, mock_git_install_logger, True, mock_workspace),
    )
...
def test_install_git_dne(mock_admin_api_post_request, app_store_workspace, mocker):
    ...
    assert (
        call("App folder Directory does not exist. Creating one.")
        in mock_git_install_logger.info.mock_calls
    )
...
@pytest.mark.django_db
def test_install_git_dne_branch_error(
    mock_admin_api_post_request, app_store_workspace, mocker
):
    mock_workspace = MagicMock(path=str(app_store_workspace))
    mocker.patch(
        "tethys_apps.base.workspace.get_app_workspace", return_value=mock_workspace
    ...
    assert (
        call(f"Couldn't check out {data['branch']} branch. Attempting to checkout main")
        in mock_git_install_logger.info.mock_calls
    )  # noqa: E501
    mock_git.Repo.init.assert_called_with(app_location)
    mock_git.Repo.init().create_remote.assert_called_with("origin", data["url"])
    ...
    mock_git.Repo.init().git.checkout.assert_has_calls(
        [call(data["branch"], "-f"), call("main", "-f")]
    )
    mock_threading.Thread.assert_called_with(
        target=mock_install_worker,
        name="InstallApps",
...
@pytest.mark.django_db
def test_run_git_install_override_no_custom_key_set(
...
@pytest.mark.django_db
def test_run_git_install_override_no_custom_key_provided(
    mock_admin_api_post_request, mocker, tmp_path
):
    mocker.patch(
        "tethysapp.app_store.git_install_handlers.get_app_workspace",
...
def test_run_git_install_override_called(mock_admin_api_post_request, mocker, tmp_path):
    ...
    mock_run_git_install_main = mocker.patch(
        "tethysapp.app_store.git_install_handlers.run_git_install_main",
        side_effect=[JsonResponse({})],
    )
    url = reverse("app_store:install_git_override")
    data = {"custom_key": override_key}

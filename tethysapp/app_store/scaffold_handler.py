...
def install_app(app_path):
    ...
    process = Popen(
        ["tethys", "install", "-d", "-q"], cwd=app_path, stdout=PIPE, stderr=STDOUT
    )
...
def scaffold_command(request, app_workspace):
    ...
    if not os.path.isdir(template_root):
        return JsonResponse(
            {
                "status": "false",
                "message": f'Error: "{template_name}" is not a valid template.',
            },
            status=500,
        )
    ...
    if os.path.isdir(project_root):
        if overwrite_if_exists:
            try:
                ...
                return JsonResponse(
                    {"status": "false", "message": error_msg}, status=500
                )
        else:
            error_msg = (
    ...
    try:
        ...
        return JsonResponse(
            {"status": "false", "message": "App scaffold failed. Check logs."},
            status=500,
        )
